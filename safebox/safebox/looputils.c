...
int loop_findfree(char *buff, size_t buffsz)
{
    ...
    for (devno = 0; devno < 256 && !found; ++devno)
    {
        for (idx = 0; loop_formats[idx] != NULL && !found; ++idx)
        {
            ...
            if (stat(loopname, &sbuff) || !S_ISBLK(sbuff.st_mode))
                continue;
...
/** Setup loopback device to point to regular file */
{
    ...
    if (ioctl(devfd, LOOP_SET_FD, filefd) || ioctl(devfd, LOOP_SET_STATUS64, &lpinfo))
    {
...
/** Find device node for given minor device number */
{
    ...
    for (idx = 0; loop_formats[idx] != NULL && !found; ++idx)
    {
        ...
        if (stat(str, &sbuff) || !S_ISBLK(sbuff.st_mode))
            continue;
        found = ((unsigned)major(sbuff.st_rdev) == maj && (unsigned)minor(sbuff.st_rdev) == min);
    }

    if (found && buff != NULL)
        strncpy(buff, str, buffsz);
