...
/* remove device-mapper target and associated device */
{
    ...
    if ((uint32_t)major(sbuff.st_rdev) == dmi.major && (uint32_t)minor(sbuff.st_rdev) == dmi.minor)
    {
        unlink(devpath);
    }
    else
...
/*! Check if device-mapper target has been setup & (optionally) get info */
{
    ...
    /* Create device-mapper target: */
    if (ident == NULL || (dmt = devmap_prepare(DM_DEVICE_INFO, ident)) == NULL || !dm_task_run(dmt) || !dm_task_get_info(dmt, dmi) || !dmi->exists)
    {
        config = 0;
    }
    if (dmt != NULL)
...
/*! Allow time for udev events to be processed */
{
    ...
    {
        if (stat(udev_mode->path, &sbuff) == 0)
            break;
        ++udev_mode;
    }
...
/*! Count number of unprocessed udev events in queue.bin file */
{
    ...
    {
        skiplen = 0;
        if (fread((void *)&seqnum, sizeof(seqnum), (size_t)1, fp) != 1 || fread((void *)&skiplen, sizeof(skiplen), (size_t)1, fp) != 1)
            break;
